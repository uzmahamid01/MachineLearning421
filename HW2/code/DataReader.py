...
def prepare_X(raw_X):
    ...
    f_symmetry = f_symmetry.reshape(-1, 1)
    ...
    f_intensity = f_intensity.reshape(-1, 1)

    # or 
    #f_intensity = raw_image.reshape(-1, 256)
	### END YOUR CODE

	# Feature 3: Bias Term. Always 1.
	### YOUR CODE HERE
    f_bias = np.ones((raw_X.shape[0], 1))
	### END YOUR CODE

	# Stack features together in the following order.
	# [Feature 3, Feature 1, Feature 2]
	### YOUR CODE HERE
    X =np.concatenate((f_bias, f_symmetry, f_intensity), axis=1)
